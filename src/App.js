...
function App() {
  ...
  useEffect(() => {
    fetch("https://maps.googleapis.com/maps/api/place/textsearch/json?query=cafe%in%Athens&key=AIzaSyDZOB6FoiTv_3JOQvtLhc2L-QW7j4wy96g", {mode: "cors"})
      .then(res => res.json())
      .then((result) => {
        setIsLoaded(true);
        setShops(result);
    ...
    <div className="App">
      ...
      <div className="mt-12 rounded-lg grid grid-cols-2 gap-4 max-w-7xl mx-auto pl-8  sm:px-16">
        ...
        {shops.map(item => (
          ...
          <div class="px-6 py-4">
            <div class="font-bold text-xl mb-2">{error.message}</div>
